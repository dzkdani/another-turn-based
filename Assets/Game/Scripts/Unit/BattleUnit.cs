...
public class BattleUnit : MonoBehaviour
{
    ...
    public Action<BattleUnit, BattleUnit, float> OnTakeDamage;
    ...
    private void Awake()
    {
        ...
        if (visual == null)
            Debug.LogWarning($"{name} is missing BattleUnitVisual component.");

        if (animationBridge == null)
            Debug.LogWarning($"{name} is missing BattleAnimationBridge component.");
    }
    ...
    private void SpawnVisual()
    {
        ...
        {
            Debug.LogWarning($"{name} has no VisualPrefab assigned in UnitSO.");
            return;
        }
    ...
    public void TakeDamage(BattleUnit attacker, float rawDamage)
    {
        if (IsDead)
            return;

        float finalDamage =
            Mathf.Max(1f, rawDamage - Data.CurrentDef);

        Data.CurrentHP -= finalDamage;
        Data.CurrentHP =
            Mathf.Clamp(Data.CurrentHP, 0f, Data.MaxHP);

        BattleEvents.OnUnitDamaged?.Invoke(this, attacker, finalDamage);

        OnTakeDamage?.Invoke(attacker, this, finalDamage);
